...
APP_OPTS="-DappName=icharge-individuation -Dlogdir=../logs -Dloader.path=../lib/,../conf/"

cd $bin_abs_path

nohup $JAVA $JAVA_OPTS $APP_OPTS -jar $(ls $base/lib/icharge-individuation-*.jar) > /dev/null 2>&1 &
echo $! > $base/bin/pid

echo "icharge-individuation is running"
