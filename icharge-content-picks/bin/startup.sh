...
APP_OPTS="-DappName=icharge-content-picks -Dlogdir=../logs -Dloader.path=../lib/,../conf/"

cd $bin_abs_path

nohup $JAVA $JAVA_OPTS $APP_OPTS -jar $(ls $base/lib/icharge-content-picks-*.jar) > /dev/null 2>&1 &
echo $! > $base/bin/pid

echo "icharge-content-picks is running"
