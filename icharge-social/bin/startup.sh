...
APP_OPTS="-DappName=icharge-social -Dlogdir=../logs -Dloader.path=../lib/,../conf/"

cd $bin_abs_path

nohup $JAVA $JAVA_OPTS $APP_OPTS -jar $(ls $base/lib/icharge-social-*.jar) > /dev/null 2>&1 &
echo $! > $base/bin/pid

echo "icharge-social is running"
