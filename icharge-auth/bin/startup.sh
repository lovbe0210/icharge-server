...
APP_OPTS="-DappName=icharge-auth -Dlogdir=../logs -Dloader.path=../lib/,../conf/"

cd $bin_abs_path

nohup $JAVA $JAVA_OPTS $APP_OPTS -jar $(ls $base/lib/icharge-auth-*.jar) > /dev/null 2>&1 &
echo $! > $base/bin/pid

echo "icharge-auth is running"
